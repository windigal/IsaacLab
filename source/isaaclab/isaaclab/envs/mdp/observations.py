...
def base_euler_xyz(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")):
    ...
    euler = torch.stack(math_utils.euler_xyz_from_quat(quat), dim=1)
    euler[euler>torch.pi] -= 2 * torch.pi
    return euler

def frictions(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot", body_names=["base_link"])):
    asset: RigidObject = env.scene[asset_cfg.name]
    return asset.root_physx_view.get_material_properties()[:, 0, :2].squeeze(1).to(env.device)

def mass(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")):
    asset: RigidObject = env.scene[asset_cfg.name]
    return torch.sum(asset.root_physx_view.get_masses(), dim=1).unsqueeze(1).to(env.device) / 30.

def contract_mask(env: ManagerBasedRLEnv, sensor_cfg: SceneEntityCfg):

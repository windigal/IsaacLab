...
from .velocity_env_cfg_leju_amp_test import LocomotionVelocityHighFreqRoughEnvCfg

##
# Pre-defined configs
##
from isaaclab_assets import LejuKuavo42_AMP_CFG  # isort: skip

@configclass
class LejuAMPRewards():
...
def ones_reward_test(env):
    """测试用的零奖励函数"""
    return torch.full((env.num_envs,), 100, dtype=torch.float32, device="cuda")

@configclass
class LejuAMPRewardstest():
    test = RewTerm(
        func=ones_reward_test,
        weight=1
    )
  
@configclass
class LejuAMPRoughEnvCfg(LocomotionVelocityHighFreqRoughEnvCfg):
    """ Leju v2: Remove arm joints."""
    rewards = LejuAMPRewards()
    # rewards = LejuAMPRewardstest()
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
        # Scene
        self.scene.robot = LejuKuavo42_AMP_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")

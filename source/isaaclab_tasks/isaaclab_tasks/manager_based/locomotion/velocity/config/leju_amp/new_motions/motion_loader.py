...
class MotionLoader:
    ...
    def __init__(self, motion_file_id: str, device: torch.device, index: str) -> None:
        ...
        self._body_names = data["body_names"][:-2]

        self.dof_positions = torch.tensor(data["dof"], dtype=torch.float32, device=self.device)[:,:-2]
    ...
    def _interpolate(
        ...
        *,
        ...
        if a.ndim >= 3:
            blend = blend.unsqueeze(-1)
        return (1.0 - blend) * a + blend * b # type: ignore
    ...
    def _slerp(
        ...
        *,
        ...
        cos_half_theta = (
            q0[..., qw] * q1[..., qw]    # type: ignore
            + q0[..., qx] * q1[..., qx] # type: ignore
            + q0[..., qy] * q1[..., qy] # type: ignore
            + q0[..., qz] * q1[..., qz] # type: ignore
        )
        ...
        ratio_a = torch.sin((1 - blend) * half_theta) / sin_half_theta   # type: ignore
        ratio_b = torch.sin(blend * half_theta) / sin_half_theta   # type: ignore
    ...
    def sample_times(self, num_samples: int, duration: float | None = None) -> np.ndarray:
        ...
        return duration * np.random.uniform(low=0.0, high=1.0, size=num_samples)  # type: ignore
...
if __name__ == "__main__":
    ...
    parser.add_argument("--index", type=int, default=0, help="Motion index")
    ...
    print(motion.body_rotations.shape)
    with open("./test/root_lin_vel.txt", "w") as f:
        f.write(str(motion.body_linear_velocities[:, 0]))
    with open("./test/root_ang_vel.txt", "w") as f:
        f.write(str(motion.body_angular_velocities[:, 0]))

    with open("./test/motion_dof_positions.txt", "w") as f:
        f.write(str(motion.dof_positions))
    with open("./test/motion_dof_velocities.txt", "w") as f:
        f.write(str(motion.dof_velocities))

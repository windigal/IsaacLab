...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        phase = ObsTerm(func=mdp.command_phase, params={"cycle_time": 0.64})
    ...
    class CriticCfg(ObsGroup):
        ...
        joint_vel = ObsTerm(func=mdp.joint_vel_rel, noise=Unoise(n_min=-1.5, n_max=1.5))
        actions = ObsTerm(func=mdp.last_action, clip=(-20.0, 20.0))
        ref_dof_pos = ObsTerm(func=mdp.compute_ref_state_constant, params={"cycle_steps": 64})
        base_lin_vel = ObsTerm(func=mdp.base_lin_vel, noise=Unoise(n_min=-0.1, n_max=0.1))
        base_ang_vel = ObsTerm(func=mdp.base_ang_vel, noise=Unoise(n_min=-0.2, n_max=0.2))
        base_euler_xyz = ObsTerm(func=mdp.base_euler_xyz)
        frictions = ObsTerm(func=mdp.frictions)
        mass = ObsTerm(func=mdp.mass)
        stance_mask = ObsTerm(func=mdp.get_gait_phase, params={"cycle_steps": 64})
...
class RewardsCfg:
    ...
    joint_deviation_arms = RewTerm(func=mdp.joint_deviation_l1, weight=-1.0, params={"joint_names": ["zarm_.*"]})
...
class LocomotionVelocityHighFreqRoughEnvCfg(ManagerBasedRLEnvCfg):
    ...
    def __post_init__(self):
        ...
        self.episode_length_s = 20.0
        # simulation settings
        self.sim.dt = 0.001  # 1000 Hz (origin: 0.01)
        self.sim.render_interval = self.decimation
        self.sim.physics_material = self.scene.terrain.physics_material
